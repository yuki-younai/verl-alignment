...
from examples.reward_function.simplerl.parser import extract_answer as qwen_extract_answer

def extract_answer(passage: str) -> str:
    if "\\boxed" in passage:
        return extract_boxed_answer(passage)

    predict_answer = qwen_extract_answer(passage, data_name="math")
...
import random
def extract_answer4mcq(passage: str) -> str:
    if "\\boxed" in passage:
        predict_answer = extract_boxed_answer(passage)
    else:
        predict_answer = qwen_extract_answer(passage, data_name="math")
        
    if predict_answer==None:
        return random.choice(['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J']), False
    #if len(predict_answer)>1:
    for option in ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'f','g','h','i','j']:
            if option in predict_answer:
                return option, True
    return random.choice(['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J']), False


def compute_score_mcq_eval(data_source, solution_str, ground_truth, extra_info=None, method='strict'):
    model_answer, is_extract_answer = extract_answer4mcq(solution_str)
    fast = False
    if not is_extract_answer:
        box_match = 0
        formatted = 0
        model_answer = "None"
        is_correct = False
...
def compute_score_math_eval(data_source, solution_str, ground_truth, extra_info=None, method='strict'):
...
def compute_score_math(data_source, solution_str, ground_truth, extra_info=None, method='strict'):
    model_answer = extract_answer(solution_str)
    fast = True
    if model_answer is None:
        box_match = -1
