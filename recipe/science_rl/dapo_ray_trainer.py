...
class RayDAPOTrainer(RayPPOTrainer):
    ...
    def fit(self):
        ...
        num_gen_batches = 0
        print("######################################################################")
        print(self.config.algorithm.filter_groups.enable)
        print(self.config.algorithm.filter_groups.max_num_gen_batches)
        print(self.config.algorithm.filter_groups.metric)
        print(self.config.trainer.save_freq)
        print("Hello World")
        print("######################################################################")
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if not self.config.algorithm.filter_groups.enable:
                        ...
                        print("[Prompt Batch Size]:", num_prompt_in_batch, len(batch), prompt_bsz)
                        if num_prompt_in_batch < prompt_bsz:
                            print(f"{num_prompt_in_batch=} < {prompt_bsz=}")
                            max_num_gen_batches = self.config.algorithm.filter_groups.max_num_gen_batches
                            if max_num_gen_batches <= 0 or num_gen_batches < max_num_gen_batches:
                                print(f"{num_gen_batches=}. Keep generating...")
                                print("[Save step]", self.global_steps, self.config.trainer.save_freq)
