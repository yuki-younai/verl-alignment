...
class MegatronModelMerger(BaseModelMerger):
    ...
    def __init__(self, config: ModelMergerConfig):
        ...
        self.params_mapping = {
            ...
            "mlp.router": "mlp.gate",
            "mlp.shared_experts.linear_fc1": "mlp.shared_expert.gate_up_proj",
            "mlp.shared_experts.linear_fc2": "mlp.shared_expert.down_proj",
            "mlp.shared_experts.gate_weight": "mlp.shared_expert_gate.weight",
            "mlp.experts.linear_fc1": "mlp.experts.*.gate_up_proj",
            "mlp.experts.linear_fc2": "mlp.experts.*.down_proj",
            # output
    ...
    def _load_state_dicts(self, model_ckpt_path: str) -> Dict[str, Any]:
        ...
        tf_config.use_cpu_initialization = True
    ...
    def _check_megatron_state_key(self, key: str) -> bool:
        ...
        skip_checking_keys = ["embedding.word_embeddings"]
    ...
    def _replace_name(self, megatron_name: str, name_mapping: dict[str, str]) -> str:
        for m_name, v_name in name_mapping.items():
            ...
            if "mlp.experts.linear" in megatron_name:
                idx = megatron_name[-1]
                param_name = param_name[:-1]
                param_name = param_name.replace('*',idx)
            return param_name
